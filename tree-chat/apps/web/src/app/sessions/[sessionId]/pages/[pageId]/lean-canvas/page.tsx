...
import { useCanvasSuggestions } from '@/components/canvas-suggestions';
...
function LeanCanvasContent() {
  ...
  // サジェスト機能のhook
  const { suggestions, isLoading, applySuggestion, dismissSuggestion } = useCanvasSuggestions({
    sessionId,
    pageId,
    businessName,
    canvasData
    ...
    const initializePage = async () => {
      try {
        ...
        if (!typeExists) {
          ...
          // 共有データの初期化
          await fs.write(`${sharedPath}/business_name`, '');
        }
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden p-4">
      ...
      {/* Business Name Input - FSTextInputを使用 */}
      ...
      <div className="fixed bottom-2 left-2 text-xs text-gray-400 font-mono">
        Session: {sessionId.slice(0, 8)}... | Page: {pageId}
      </div>
    </div>
  );
}
